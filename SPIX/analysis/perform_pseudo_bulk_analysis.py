...
try:
    ...
    sce = None
    print("Warning: scanpy.external not found. Harmony integration will not be available.")
...
def perform_pseudo_bulk_analysis(
    ...
    batch_key : str or None, optional (default=None)
        ...
        and used as the `batch_key` for HVG calculation and the `key` for
        Harmony integration (if `perform_pca` is True). Set to None to disable
    ...
    if perform_pca and batch_key is not None and sce is None:
         print("Warning: Harmony integration requested via batch_key but scanpy.external (needed for harmony) is not available.")


    print("--- Starting Pseudo-Bulk Aggregation and Analysis ---")
    print(f"Segmenting based on key: '{segment_key}'")
    if batch_key:
        print(f"Batch key specified for correction: '{batch_key}'")
    ...
    print("Aggregating spatial coordinates (mean)...")
    ...
    print("Aggregating expression data (mean)...")
    # Use a more memory-efficient approach for sparse matrices
    if issparse(adata_X):
        print("Input data is sparse. Using sparse-aware aggregation.")
        ...
        print("Input data is dense. Using pandas groupby aggregation.")
    ...
    print("Creating new AnnData object for pseudo-bulk data...")
    ...
    print("New AnnData object initialized with spatial coordinates.")
    ...
    print("Aggregated mean counts added to 'counts' layer.")

    # Add batch_key from original adata.obs if provided and present
    if batch_key and batch_key in adata.obs.columns:
        print(f"Adding '{batch_key}' to new_adata.obs...")
        try:
            ...
            print(f"'{batch_key}' added.")
            del segment_mapping_df, batch_ids
            gc.collect()
        except Exception as e:
             print(f"Warning: Failed to map and add '{batch_key}' to new_adata.obs. Error: {e}")
             # Ensure batch_key is None if mapping failed so downstream steps skip batch correction
    ...
    if add_bulked_layer_to_original_adata:
        print("Adding bulked expression layer to original adata...")
        try:
            ...
            if issparse(bulked_mean_X):
                 print("Bulked data is sparse. Using sparse-aware method to add layer to original adata.")
                ...
                print("Bulked data is dense. Using dense indexing.")
            ...
            print("'bulked' layer added to original adata.")

        except Exception as e:
            print(f"Warning: Failed to add 'bulked' layer to original adata. Error: {e}")
            # Continue with the rest of the analysis

    # --- Preprocessing Steps on new_adata ---
    print("\n--- Preprocessing Pseudo-Bulk Data ---")
    print(f"Filtering genes: Keeping genes detected in at least {min_genes_in_segment} segments.")
    ...
    print(f"Gene filtering complete. Retained {new_adata.n_vars} genes (removed {initial_genes - new_adata.n_vars}).")


    if normalize_total:
        print("Normalizing total counts to 1e4...")
    ...
    print("\n--- Computing Spatial Neighbors and Autocorrelation ---")
    print("Computing spatial neighbors on pseudo-bulk data using 'spatial' coordinates...")
    ...
    if 'spatial_neighbors' not in new_adata.uns:
         ...
         print("Error: Spatial neighbors graph not found in new_adata.uns. Cannot compute spatial autocorrelation.")
         return new_adata, pd.DataFrame()
    ...
    try:
        ...
        print("Spatial autocorrelation calculation complete.")
    except Exception as e:
         print(f"Error calculating spatial autocorrelation: {e}")
    ...
    if results_key in new_adata.uns and not new_adata.uns[results_key].empty:
        ...
        if index_col in results_df.columns:
             ...
             print(f"Number of genes with {mode}'s {index_col} > {moranI_threshold}: {superpixel_moranI.shape[0]}")
        else:
             print(f"Warning: Expected index column '{index_col}' not found in '{results_key}' results.")

    else:
        print(f"Warning: Spatial autocorrelation results ('{results_key}') not found in new_adata.uns or are empty.")


    # --- Additional Pseudo-Bulk Preprocessing (HVG, PCA, Neighbors) ---
    print("\n--- Additional Pseudo-Bulk Preprocessing (HVG, PCA) ---")
    # These steps use new_adata.X which is normalized/log1p data

    # Determine if batch correction steps should be performed
    perform_batch_correction = False
    if batch_key and batch_key in new_adata.obs.columns:
        # Check if there's more than one unique batch
        if len(new_adata.obs[batch_key].unique()) > 1:
            perform_batch_correction = True
            print(f"Multiple batches detected using key '{batch_key}'. Batch correction steps will be attempted.")
        else:
            print(f"Only one unique value found in '{batch_key}'. Skipping batch correction steps.")
    else:
        print("No batch key specified or key not found in new_adata.obs. Skipping batch correction steps.")

    if highly_variable:
        print("Calculating Highly Variable Genes...")
        if perform_batch_correction:
            print(f"Using '{batch_key}' for batch-aware HVG calculation.")
            try:
                sc.pp.highly_variable_genes(new_adata, batch_key=batch_key, inplace=True)
                 ...
                 print("Warning: Batch-aware HVG failed (possibly due to old scanpy version). Calculating HVGs without batch key.")
                 sc.pp.highly_variable_genes(new_adata, inplace=True)
            except Exception as e:
                 print(f"Warning: Batch-aware HVG failed: {e}. Calculating HVGs without batch key.")
                 sc.pp.highly_variable_genes(new_adata, inplace=True)
        else:
            sc.pp.highly_variable_genes(new_adata, inplace=True)
    ...
    if perform_pca:
        ...
        if not use_hvg:
             print("Warning: No highly variable genes computed or found. PCA will use all genes.")
        ...
        try:
             ...
             if perform_batch_correction:
                 if sce is not None and hasattr(sce, 'harmony_integrate'):
                     print(f"Attempting Harmony integration using '{batch_key}'...")
                     try:
                        ...
                        print("Harmony integration complete. X_pca replaced with X_pca_harmony.")
                     except Exception as e:
                        print(f"Warning: Harmony integration failed: {e}")
                        print("Using original X_pca.")
                 else:
                      print("Warning: Harmony integration requested but scanpy.external.harmony_integrate is not available.")
                      print("Using original X_pca.")
             else:
                print("Skipping Harmony integration.")


             # Compute neighbors on the PCA space
             print("Computing neighbors on PCA space...")
             ...
             print(f"Error during PCA or Neighbors computation: {e}")
             print("Skipping PCA and Neighbors.")
