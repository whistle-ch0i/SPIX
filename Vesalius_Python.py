...
def is_collinear(points, tol=1e-8):
    ...
    if len(points) < 3:
        return True
    # Compute the area of triangles formed by the first two points and each subsequent point
...
def is_almost_collinear(points, tol=1e-5):
    """
    Check if a set of points are almost collinear.
    ...
    if len(points) < 3:
        return True
    # Use linear regression to check the fit
    x = points[:, 0]
    y = points[:, 1]
